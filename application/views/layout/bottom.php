...
<script type="text/javascript" src="<?= base_url('') ?>plantilla/assets/js/plugins/forms/selects/select2.min.js">
</script>
...
jQuery(function($) {
  ...
  $('#btn-dbf').on('click', function(e) {
    ...
    window.open("<?= base_url() ?>" + "prueba?tipo=dbf&id=1&mes=" + mes + "&ano=" + ano + "&doc=" + doc,
      "_blank");
  });
  var allow = false;
  ...
  $('#btn-kardex-excel').on('click', function(e) {
    ...
    var mes = $("#mes_kardex").val();
    var ano = $("#ano_kardex").val();
    window.open("<?= base_url() ?>" + "prueba?tipo=kardex_excel&id=" + id + "&mes=" + mes + "&ano=" + ano,
      "_blank");
  });
  $('#btn-venta-reporte').on('click', function(e) {
    ...
    var ano = $("#ano_concar").val();
    var doc = $("#doc_concar").val();
    window.open("<?= base_url() ?>" + "prueba?tipo=venta&id=1&mes=" + mes + "&ano=" + ano + "&doc=" + doc,
      "_blank");
  });
  $('#btn-concar').on('click', function(e) {
    ...
    var ano = $("#ano_concar").val();
    var doc = $("#doc_concar").val();
    window.open("<?= base_url() ?>" + "prueba?tipo=concar&id=1&mes=" + mes + "&ano=" + ano + "&doc=" + doc,
      "_blank");
  });
  ...
  function addSerie(quantity) {
    ...
    for (var i = 0; i < quantity; i++) {
      ...
      var serie = '<div class="form-group row serieItem" style="padding-left: 10px"> ' +
        '<label class="col-sm-2 col-form-label col-form-label" style="background: #F2F2F2">#' + ((currentItem + 1) +
          "").padStart(2, "0") + ' Serie: </label> ' +
        '<div class="col-sm-9"> <input type="text" name="serie[]" class="form-control" required> </div> ' +
...
$(function() {
  ...
  $('#frm-cliente-coti').on('submit', function(e) {
    ...
    $.ajax({
      ...
      if (data != null) {
        ...
        $('#cliente_id').append("<option value = '" + data + "' tipo='" + $('#tipo').val() + "'>" + $(
            '#nombre').val() +
          " - " + $('#tipo_doc').val() + " - " + $('#numero_doc').val() + $('#direccion').val() +
          "</option>");
        $('#numero_doc').val("");
        $('#nombre').val("") + "-"
  ...
  $('#frm-add-acabado').on('submit', function(e) {
    ...
    $.ajax({
      ...
      if (data != null) {
        ...
        $('#acabado').append("<option value = '" + data + "' precio='" + $('#precio_acabado_crear').val() +
  ...
  $('#btn-reporte-clientes').on('click', function(e) {
    ...
    window.open("<?= base_url() ?>" + "prueba?tipo=clientes&id=1&fechainicio=" + fechainicio + "&fechafin=" +
      fechafin, "_blank");
  });
  $('#btn-reporte-pendientes').on('click', function(e) {
    ...
    window.open("<?= base_url() ?>" + "prueba?tipo=ventas_pendientes&id=1&fechainicio=" + fechainicio +
      "&fechafin=" + fechafin + "&disenador=" + disenador + "&estado=" + estado, "_blank");
  });
  $('#btn-dbf').on('click', function(e) {
    var mes = $("#mes_concar").val();
    var ano = $("#ano_concar").val();
  ...
  $('#cotizacion_id').on('change', function(e) {
    if ($(this).val() != '') {
      ...
      if ($('#formapago').val() == "CONTADO") {
        ...
        $('#vuelto').val(parseFloat(parseFloat($('#recibido').val()) - parseFloat($('#total').val())).toFixed(
          2));
      } else if ($('#formapago').val() == "CREDITO") {
        if ($('#adelanto').val() <= 0 || $('#adelanto').val() == "") {
          $('#adelanto').val("0.00");
        }
        $('#saldo').val(parseFloat(parseFloat($('#total').val()) - parseFloat($('#adelanto').val())).toFixed(
        2));
      }
  ...
  function consultar_dni(dni) {
    ...
    $.ajax({
      url: 'https://cors-anywhere.herokuapp.com/http://aplicaciones007.jne.gob.pe/srop_publico/Consulta/Afiliado/GetNombresCiudadano?DNI=' +
        dni,
      ...
      },
      {
        ...
        render: function(data) {
          if (data.dias_transcurridos >= 7) {
            ...
            return '<label class="label label-warning btn-generarxml" data-serie="' + data.serie +
              '" data-numero="' + data.numero + '" data-id="' + data.id +
              '" data-tipo="factura">Generar XML</label> ';
          }
        }
      },
      {
        ...
        render: function(data) {
          if (data.dias_transcurridos >= 7) {
            ...
            return '<label   class="label label-warning btn-pendiente" data-serie="' + data.serie +
              '" data-numero="' + data.numero + '" data-id="' + data.id +
              '" data-tipo="factura">Pendiente</label> ';
          }
        }
      },
      {
      ...
      },
      {
        ...
        render: function(data) {
          if (data.dias_transcurridos >= 7) {
            ...
            return '<label class="label label-warning btn-generarxml" data-serie="' + data.serie +
              '" data-numero="' + data.numero + '" data-id="' + data.id +
              '" data-tipo="boleta">Generar XML</label> ';
          }
        }
      },
      {
        ...
        render: function(data) {
          if (data.dias_transcurridos >= 7) {
            ...
            return '<label class="label label-warning btn-pendiente" data-serie="' + data.serie +
              '" data-numero="' + data.numero + '" data-id="' + data.id +
              '" data-tipo="boleta">Pendiente</label> ';
          }
        }
      },
      {
...
$('#frm-add-producto-remision').on('submit', function(e) {
  ...
  if (updating) {
    ...
    $('#tbody_producto_remision tr[data-product-id=' + jqxhr + ']').attr('data-product', JSON.stringify(dat)
      .replace(/'/g, "\\'"));
  } else {
    $('#tbody_producto_remision').append('<tr data-product=\'' + JSON.stringify(dat).replace(/'/g, "\\'") +
      '\' data-product-id="' + jqxhr + '">' + trHtml + '</tr>');
  }
...
function CargarUbigeo() {
  ...
  $.get('https://ubigeo.escienza.pe/api/v1/departaments', function(r) {
    if (r.success) {
  ...
  sel_departamento_partida.on('change', function() {
    ...
    ubigeo_partida.val('');
    $.get('https://ubigeo.escienza.pe/api/v1/departaments/' + codigoDepartamento + '/provinces', function(r) {
      if (r.success) {
  ...
  sel_provincia_partida.on('change', function() {
    ...
    sel_distrito_partida.html('');
    ubigeo_partida.val('');
    $.get('https://ubigeo.escienza.pe/api/v1/departaments/' + codigoDepartamento + '/provinces/' + codigoProvincia +
      '/districts',
      function(r) {
        if (r.success) {
