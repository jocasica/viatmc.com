								...
								<tbody>
									...
									?>
										<tr>
											...
											<td>
												<label class="label label-success cursor_pointer">
													<a type="button" onclick="verificar_cotizacion('<?php echo $c->id; ?>')" title="Clic aquí para verificar la cotización">
														APROBADO
													</a>
												</label>
											</td>
											<td>
												...
												<!-- Verificar -->
												<a href="<?php echo base_url('cotizacion/verificar/').$c->id; ?>" class="btn btn-success btn-xs" title="Verificar productos cotización"><i class="fa fa-check"></i> Verificar</a>
											</td>
										</tr>
									<?php endforeach; ?>
								</tbody>
...
<script>
	...
	const verificar_cotizacion = (id) => {
		$('#modal_verificar_cotizacion').modal('show');
		...
		.then((res) => {
			if(res) {
				window.location = 'cotizacion/guia_remision/'+id;
			}
		}); */
	};
</script>