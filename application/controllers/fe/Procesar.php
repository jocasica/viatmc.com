...
class Factura extends CI_Controller {
	...
	public function index()
	{
		$this->load->model(array(
			'Cliente_model',
			'Cliente_producto_model',
		));
		$validacion = new Validaciondedatos();
	$data = $_POST;

	$id_cliente = $this->Cliente_model->obtener_id($data['cliente_nombre']);
	if ($data['cliente_direccion'] == 0) {
		$data['cliente_direccion'] = $this->Cliente_model->obtener_direccion_principal($id_cliente);
	}//end if
	if ($data['cliente_direccion'] > 0) {
		$data['cliente_direccion'] = $this->Cliente_producto_model->obtener_direccion($data['cliente_direccion']);
	}//end if
