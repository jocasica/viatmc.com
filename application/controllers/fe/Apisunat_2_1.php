...
class Apisunat {
    ...
    public function crear_xml_factura($cabecera, $detalle, $ruta) {
        ...
        $doc->encoding = 'utf-8';
        $ubigeoEmpresa = $cabecera['CODIGO_UBIGEO_EMPRESA'];
...
<Invoice xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ccts="urn:un:unece:uncefact:documentation:2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:qdt="urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2" xmlns:udt="urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2" xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2">
	...
	<cac:LegalMonetaryTotal>
		...
		<cac:Item>
            ...
            if (isset($cabecera['PLACA'])) {
                if (!empty($cabecera['PLACA'])) {
                    $xmlCPE = $xmlCPE . '<cac:AdditionalItemProperty><cbc:Name>' . $cabecera['PLACA'] . '</cbc:Name></cac:AdditionalItemProperty>';
                }
            }
...
<cbc:PriceAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $detalle[$i]["txtPRECIO_DET"] . '</cbc:PriceAmount>
    ...
    public function crear_xml_resumen_documentos($cabecera, $detalle, $ruta) {
        ...
        try {
            ...
            <cbc:ID>RC-' . $cabecera["SERIE"] . '-' . $cabecera["SECUENCIA"] . '</cbc:ID>
    ...
    public function enviar_documento($ruc, $usuario_sol, $pass_sol, $ruta_archivo, $ruta_archivo_cdr, $archivo, $ruta_ws) {
        $respuesta['error'] = true;
        $respuesta['cod_sunat'] = '0';
        $respuesta['cod_ticket'] = '';
        $respuesta['mensaje'] = '';
        $respuesta['extra'] = '';
        $respuesta['respuesta'] = '';
        $respuesta['hash_cdr'] = '';
        try {
            ...
            if (!file_exists($ruta_archivo . '.ZIP')) {
                throw new Exception('No existe XML');
            }
            ...
            if ($httpcode == 200) {
                ...
                if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) {
                    ...
                    if ($zip->open($ruta_archivo_cdr . 'R-' . $archivo . '.ZIP') === TRUE) {
                        ...
                        $zip->extractTo($ruta_archivo_cdr, 'R-' . $archivo . '.xml'); //nubefact
                        $zip->close();
                        @rename($ruta_archivo_cdr . 'R-' . $archivo . '.xml', $ruta_archivo_cdr . 'R-' . $archivo . '.XML');
                    }
                    ...
                    $respuesta['cod_sunat'] = $doc_cdr->getElementsByTagName('ResponseCode')->item(0)->nodeValue;
                    $respuesta['mensaje'] = $doc_cdr->getElementsByTagName('Description')->item(0)->nodeValue;
                    $respuesta['hash_cdr'] = $doc_cdr->getElementsByTagName('DigestValue')->item(0)->nodeValue;
                } else {
                    $respuesta['respuesta'] = 'error';
                    $respuesta['cod_sunat'] = $doc->getElementsByTagName('faultcode')->item(0)->nodeValue;
                    $respuesta['mensaje'] = $doc->getElementsByTagName('faultstring')->item(0)->nodeValue;
                    $respuesta['hash_cdr'] = "";
                }
            } else {
                throw new Exception('Web Service no disponible(ENVIO:DOCUMENTO).');
            }
        } catch (Exception $exc) {
            $respuesta['mensaje'] = $exc->getMessage();
        }
    ...
    <soapenv:Body>
        <ser:sendBill>
            ...
            if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) {
                ...
                $respuesta['cod_sunat'] = $doc_cdr->getElementsByTagName('ResponseCode')->item(0)->nodeValue;
                $respuesta['msj_sunat'] = $doc_cdr->getElementsByTagName('Description')->item(0)->nodeValue;
                $respuesta['hash_cdr'] = $doc_cdr->getElementsByTagName('DigestValue')->item(0)->nodeValue;
            } else {
                //$respuesta['cod_sunat'] = $doc->getElementsByTagName('faultcode')->item(0)->nodeValue;
                //$respuesta['msj_sunat'] = $doc->getElementsByTagName('faultstring')->item(0)->nodeValue;
                //$respuesta['hash_cdr'] = "";

                $respuesta['cod_sunat'] = $doc->getElementsByTagName('faultstring')->item(0)->nodeValue;
                $respuesta['msj_sunat'] = $doc->getElementsByTagName('message')->item(0)->nodeValue;
                $respuesta['hash_cdr'] = "";
            }
        } catch (Exception $e) {
            $respuesta['cod_sunat'] = "0000";
            $respuesta['msj_sunat'] = "SUNAT ESTA FUERA SERVICIO: " . $e->getMessage();
            $respuesta['hash_cdr'] = "";
        }
        //print_r($respuesta); 
        return $respuesta;
    ...
    public function crear_xml_guia_remision($cabecera, $detalle, $ruta) {
        ...
        <DespatchAdvice xmlns="urn:oasis:names:specification:ubl:schema:xsd:DespatchAdvice-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2">
            ...
            <cac:Shipment>
                ...
                <cac:ShipmentStage>
                    ...
                    <cac:TransportMeans>
                        <cac:RoadTransport>
                            <cbc:LicensePlateID>' . $cabecera["CHOFER_LICENCIA"] . '</cbc:LicensePlateID>
                        </cac:RoadTransport>
                    </cac:TransportMeans>
                    <cac:DriverPerson>
                        <cbc:ID schemeID="1">' . $cabecera["CHOFER_DOCUMENTO"] . '</cbc:ID>
                    </cac:DriverPerson>
                </cac:ShipmentStage>
                <cac:Delivery>
                    <cac:DeliveryAddress>
        ...
        for ($i = 0; $i < count($detalle); $i++) {
            $xmlCPE= $xmlCPE . '<cac:DespatchLine>
                        <cbc:ID>'.($i+1).'</cbc:ID>
                        <cbc:DeliveredQuantity unitCode="NIU">' . $detalle[$i]["CANTIDAD"] . '</cbc:DeliveredQuantity>
    ...
    public function crear_xml_baja_sunat($cabecera, $detalle, $ruta) {
        ...
        <cbc:UBLVersionID>2.0</cbc:UBLVersionID>
        <cbc:CustomizationID>1.0</cbc:CustomizationID>
        <cbc:ID>RA-' . $cabecera["SERIE"] . '-' . $cabecera["SECUENCIA"] . '</cbc:ID>
        <cbc:ReferenceDate>' . $detalle[0]["FECHA"] . '</cbc:ReferenceDate>
        <cbc:IssueDate>' . $cabecera["FECHA_DOCUMENTO"] . '</cbc:IssueDate>
        <cac:Signature>
        ...
        <cbc:RegistrationName><![CDATA[' . $cabecera["RAZON_SOCIAL_EMPRESA"] . ']]></cbc:RegistrationName>
            ...
            for ($i = 0; $i < count($detalle); $i++) {
                ...
                <sac:VoidReasonDescription><![CDATA[' . $detalle[$i]["MOTIVO"] . ']]></sac:VoidReasonDescription>
    ...
    public function enviar_documento_para_baja($ruc, $usuario_sol, $pass_sol, $ruta_archivo, $ruta_archivo_cdr, $archivo, $ruta_ws) {
        ...
        try {
            ...
            if ($httpcode == 200) {
                ...
                throw new Exception('Web Service no disponible(ENVIO:BAJA).');
            }
        } catch (Exception $exc) {
            $respuesta['mensaje'] = $exc->getMessage();
        }
    ...
    public function enviar_resumen_boletas($ruc, $usuario_sol, $pass_sol, $ruta_archivo, $ruta_archivo_cdr, $archivo, $ruta_ws) {
        ...
        try {
            ...
            $filenameXMLCPE = $ruta_archivo . '.ZIP';
            if ($zip->open($filenameXMLCPE, ZIPARCHIVE::CREATE) === true) {
                $zip->addFile($ruta_archivo . '.XML', $archivo . '.XML');
                $zip->close();
            }
            $soapUrl = $ruta_ws;
            // xml post structure
            ...
            if ($httpcode == 200) {//======LA PAGINA SI RESPONDE
                ...
                $doc = new DOMDocument();
                $doc->loadXML($response);
                //print_r($doc->getElementsByTagName('ticket')->item(0)->nodeValue);
                //===================VERIFICAMOS SI HA ENVIADO CORRECTAMENTE EL COMPROBANTE=====================
                if (isset($doc->getElementsByTagName('ticket')->item(0)->nodeValue)) {
                    $ticket = $doc->getElementsByTagName('ticket')->item(0)->nodeValue;
                    unlink($ruta_archivo . '.ZIP');
                ...
                throw new Exception('Web Service no disponible(ENVIO:RESUMEN).');
            }
        } catch (Exception $exc) {
            $respuesta['mensaje'] = $exc->getMessage();
        }
    ...
    public function consultar_envio_ticket($ruc, $usuario_sol, $pass_sol, $ticket, $archivo, $ruta_archivo_cdr, $ruta_ws) {
        ...
        try {
            ...
            if ($httpcode == 200) {
                ...
                $doc->loadXML($response);
                //===================VERIFICAMOS SI HA ENVIADO CORRECTAMENTE EL COMPROBANTE=====================
                //print_r($doc->getElementsByTagName('content')->item(0)->nodeValue);
                ...
                throw new Exception('Web Service no disponible(CONSULTA:TICKET).');
            }
        } catch (Exception $exc) {
            $respuesta['mensaje'] = $exc->getMessage();
        }
